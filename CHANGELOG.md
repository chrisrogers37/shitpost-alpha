...
- **Echo embedding TOCTOU race (Phase 5, #231 L16)** — `EchoService.embed_and_store` checked existence in one session and inserted in another with an OpenAI `embed()` call between; concurrent/retried runs for the same `prediction_id` could both pass the check and race, and the second ORM insert hit the unique constraint → unhandled `IntegrityError`, event failed & retried. Replaced the ORM insert with `INSERT … ON CONFLICT (prediction_id) DO NOTHING` (name-agnostic column inference), treating a `rowcount == 0` conflict as idempotent success. Pre-check retained as a cost optimization; additive, no migration.
...
- **EventStatus constants (Phase 4, #190)** — replaced all 17 bare event-status string literals across `shit/events/` (`models.py`, `worker.py`, `cleanup.py`, `cli.py`, `producer.py`) with a new `EventStatus` class in `event_types.py` (mirroring the sibling `EventType`/`ConsumerGroup` idiom). Values stay byte-identical to the persisted strings — no DB value, index, or query semantics change — and are pinned by a new byte-identity test.
- **Timezone-aware datetimes (Phase 4, #230 L4)** — replaced 13 deprecated naive `datetime.utcnow()` sites (8 call sites + 5 SQLAlchemy callable defaults) across `notifications/`, `shit/market_data/`, and `shitvault/shitpost_models.py` with tz-aware `datetime.now(timezone.utc)`, matching the established `TimestampMixin`/`Event.mark_*` precedent. Persisted wall-clock is unchanged under the UTC Neon session; no schema/data impact. `HealthReport.to_dict()` timestamps now carry a `+00:00` isoformat suffix (intentional, externally visible; pinned by test).
- **Consolidated S3-key processing (Phase 5, #191)** — extracted the stats-building + `get_raw_data`→`_process_single_s3_data` loop + `SIGNALS_STORED` emission (duplicated in `S3Processor` and the `S3ProcessorWorker`) into a shared public `S3Processor.process_keys()` and a best-effort `_emit_signals_stored()` helper. The worker no longer reaches into the private `_process_single_s3_data` across a module boundary; its emission is now best-effort (swallow+warn), harmonized with the processor path. Stats shape and emit-once behavior preserved and tested.
- **Bounded price cache (Phase 5, #231 L18)** — `api/queries/price_queries.py`'s `_price_cache` is now an `OrderedDict` with LRU eviction past `_CACHE_MAX` (256 entries), keeping the 5-min TTL. Previously every distinct `(symbol, days)` key lived forever (unbounded memory growth).
...
### Removed
- **Dead API surface (Phase 5, #231 L18)** — removed the unused `get_db()` dependency (zero callers; it modeled a session leak), the duplicate `/latest` feed endpoint (identical to `/at?offset=0`; the React app calls only `/at`), and the empty `lifespan` handler (bare `yield`, no startup/shutdown work). Api tests repointed from `/latest` to `/at?offset=0`.
...
- **Dash 4.0 migration** — Upgraded dashboard from Dash 2.x to Dash 4.0 (`dash>=4.0.0,<5.0.0`) and `dash-bootstrap-components` from 1.x to 2.x (`>=2.0.0,<3.0.0`). DCC components now use Radix-based redesign; all existing functionality preserved.
